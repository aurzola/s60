...
class FlickrS60Thumb:
	def __init__(self, path):
		...
		self.img = graphics.Image.new((320, 240))
		self.img_tmp = graphics.Image.new((36, 36))
		self.x, self.y = 0, 0
		self.p = 0
		self.pages = 0
		self.currpages = 0
	...
	def OnUpdate(self, rect):
		...
		self.canvas.rectangle([(self.p+(42*self.x), 36*self.y), (self.p+(42*self.x)+42, (36*self.y)+36)], width=4, outline=0xFF0056)
	def move(self, x, y):
		self.x = (self.x+x)%4
		self.y = (self.y+y)
	...
	def IMG(self):
		try:
			m = self.listFile[self.mpath]
			self.img.rectangle([(0,180),(320,240)], width=2, outline=0xFF0056,fill=0xFFFFFF)
			self.img.text((0,195), m)
	...
	def _createList(self):
		try:
			...
			raise FlickrS60Error('Error creando lista de imagenes.')
if __name__ == '__main__':
#	FlickrS60Thumb('E:\\Images\\_PalbTN\\').OnRun()
	FlickrS60Thumb('c:\\data\\Images\\Cable\\').OnRun()
